...
# Initialize list for the text of all reviews and all reviews with some sentiment
review_text_list = []
review_text_list_sentiment = []

# Add sentences to list
# For sentiment list add if there is a positive or negative sentiment (abs > 0.2)
all_sentences = pd.read_csv("data/all_sentences.csv")
for i in range(0, len(all_sentences)):
    review_text_list.append(all_sentences.loc[i]["Sentence"])
    if abs(all_sentences.loc[i]["Sentiment"]) > 0.2:
        review_text_list_sentiment.append(all_sentences.loc[i]["Sentence"])
...
corp_sentiment = [preprocess(paragraph) for paragraph in review_text_list_sentiment]
...
if not os.path.exists("model"):
    os.mkdir("model")
pickle.dump(dictionary, open("model/dictionary2.pickle", "wb"))
pickle.dump(corp, open("model/corpus2.pickle", "wb"))
pickle.dump(corp_sentiment, open("model/corpus_sentiment2.pickle", "wb"))
...
corp_sentiment_df = pd.DataFrame(data={"Review List": corp_sentiment})
corp_sentiment_df.to_csv("data/cleaned_reviews_sentiment_lemmatized.csv", sep=',',index=False)
...
# Find words frequency for sentences with sentiment
all_words = []
for i in corp_sentiment:
    for j in i:
        all_words.append(j)
fdist = nltk.FreqDist(all_words)

# Plot the most common words for sentences with sentiment
sns.set(rc={'figure.figsize':(9,4)})
sns.set(rc={'axes.labelsize':24})
plt.rc('xtick', labelsize=18)    # fontsize of the tick labels
plt.rc('ytick', labelsize=18)
...
corp = pickle.load(open("model/corpus_sentiment2.pickle", "rb"))
...
pickle.dump(lda_models, open("model/lda_models.pickle", "wb"))
